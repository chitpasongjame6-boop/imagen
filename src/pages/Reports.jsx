...
import { Calendar, TrendingUp, CheckCircle, AlertCircle, Download } from 'lucide-react'
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-3 text-xs">
        ...
        {payload.map((p) => (
          <p key={p.dataKey} style={{ color: p.fill }}>
            {p.dataKey === 'ยอดขาย' ? 'ยอดขาย' : 'หนี้ค้าง'}: {formatCurrency(p.value)}
          </p>
        ))}
      </div>
    )
...
export default function Reports() {
  ...
  }).filter((a) => a.ยอดขาย > 0).sort((a, b) => b.ยอดขาย - a.ยอดขาย)
        ...
        {agentBreakdown.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={Math.max(180, agentBreakdown.length * 50)}>
            ...
            >
              ...
              <Bar dataKey="ยอดขาย" fill="#3b82f6" radius={[0, 4, 4, 0]} maxBarSize={16} />
              <Bar dataKey="หนี้ค้าง" fill="#f87171" radius={[0, 4, 4, 0]} maxBarSize={16} />
            </BarChart>
          </ResponsiveContainer>
        )}
          ...
          <div className="space-y-0">
            ...
            {agentBreakdown.map((a) => (
              <div key={a.id} className="grid grid-cols-4 text-sm py-2.5 border-b border-gray-50 last:border-0">
                ...
                <p className="text-right font-semibold text-gray-800 text-xs">{formatCurrency(a.ยอดขาย)}</p>
                <p className={`text-right font-semibold text-xs ${a.หนี้ค้าง > 0 ? 'text-red-500' : 'text-green-500'}`}>
                  {a.หนี้ค้าง > 0 ? formatCurrency(a.หนี้ค้าง) : '✓'}
                </p>
              </div>
            ))}
